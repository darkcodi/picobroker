...
impl PacketFixedSize for PubAckPacket {
...
impl core::fmt::Display for PubAckPacket {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "PubAckPacket {{ packet_id: {} }}", self.packet_id)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_puback_packet_struct_size() {
        assert_eq!(size_of::<PubAckPacket>(), 2);
    }

    #[test]
    fn test_puback_packet_roundtrip() {
        let packet = roundtrip_test(&[0x40, 0x02, 0x12, 0x34]);
        assert_eq!(packet.packet_id, 0x1234);
    }

    fn roundtrip_test(bytes: &[u8]) -> PubAckPacket {
        let result = PubAckPacket::decode(bytes);
