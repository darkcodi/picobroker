...
pub fn read_variable_length(bytes: &[u8]) -> Result<(usize, usize), PacketEncodingError> {
    ...
    loop {
        ...
        if value > MAX_VARIABLE_LENGTH {
            return Err(PacketEncodingError::InvalidPacketLength { expected: MAX_VARIABLE_LENGTH, actual: value });
        }
...
pub fn write_variable_length(value: usize, buffer: &mut [u8]) -> Result<usize, PacketEncodingError> {
    // MQTT spec limits variable length to 268,435,455 (0x0FFFFFFF)
    const MAX_VARIABLE_LENGTH: usize = 268_435_455;
...
pub fn read_string<'a>(bytes: &'a [u8], offset: &'_ mut usize) -> Result<&'a str, PacketEncodingError> {
    ...
    *offset += len;
    let str_slice = core::str::from_utf8(str_bytes).map_err(|_| PacketEncodingError::InvalidUtf8String)?;
    Ok(str_slice)
}

pub fn write_string(s: &str, buffer: &mut [u8], offset: &mut usize) -> Result<(), PacketEncodingError> {
...
pub fn read_binary<'a>(bytes: &'a [u8], offset: &'_ mut usize) -> Result<&'a [u8], PacketEncodingError> {
    if *offset + 2 > bytes.len() {
        return Err(PacketEncodingError::IncompletePacket { buffer_size: bytes.len() } );
    }
    let len = u16::from_be_bytes([bytes[*offset], bytes[*offset + 1]]) as usize;
...
pub fn write_binary(data: &[u8], buffer: &mut [u8], offset: &mut usize) -> Result<(), PacketEncodingError> {
    let len = data.len();
    if *offset + 2 + len > buffer.len() {
        return Err(PacketEncodingError::BufferTooSmall { buffer_size: buffer.len() } );
    }
